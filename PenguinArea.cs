...
public class PenguinArea : MonoBehaviour
{
   ...
   // private PenguinAcademy penguinAcademy;
    ...
    /// </summary>
    ///当企鹅抓到一条鱼时，PenguinAgent脚本将调用RemoveSpecificFish（）将其从水中移出。
    ...
    /// </summary>
    /// ResetArea（）函数调用RemoveAllFish（）来确保在产卵新鱼之前该区域没有鱼。
    ...
    /// </summary>
    /// 这些功能可以放置企鹅。在这两种情况下，它们都将刚体速度设置为零，
    /// 因为在以100倍速进行长时间训练时，可能会发生意外情况。例如，企鹅可能掉落在地板上，
    /// 然后向下加速。当区域重置时，位置将重置，但是如果向下速度未重置，则企鹅可能会通过地面爆炸。
    ...
    /// </summary>
    /// 此功能可在场景中找到企鹅学院并重置学院。请注意，场景中只能有一个PenguinAcademy，它可以控制一个或多个PenguinArea。
    private void Start()
    {
    ...
    ///======================================================================================================
    /// 此功能每帧更新区域后墙上的累积奖励显示文本。训练不是必需的，但是它可以帮助了解企鹅的表现。
    /// <summary>
    /// Called every frame
    /// </summary>
    private void Update()
